...
<div class="row">
    <div class="col-lg-6">
        <a href="<?= base_url('admin/presensi/add/'); ?>" class="btn btn-success mb-3">
            <i class="fas fa-plus-circle"></i> Tambah
        </a>
    </div>
    <div class="col-lg-6 text-right">
        <a href="<?= base_url('admin/presensi/history/'); ?>" class="btn btn-secondary mb-3">
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <?php if ($row->kegiatan == 0) { ?>
                                <form action="<?= base_url('admin/presensi/in/') ?>" method="POST">
                            ...
                            <?php if ($row->kegiatan == 1) {
                                foreach ($pres as $rowpres) :
                                    if ($rowpres->nik == $row->nik) { ?>
                                        <form action="<?= base_url('admin/presensi/out/') ?>" method="POST">
                        ...
                        <td class="text-center">
                            <?php if ($row->kegiatan == 1) {
                                foreach ($pres as $rowpres) :
                                    if ($rowpres->nik == $row->nik) { ?>
                                        <form action="<?= base_url('admin/lembur/in') ?>" method="POST">
                                            <input type="hidden" name="id_presensi" id="id_presensi" value="<?= $rowpres->id_presensi; ?>">
                                            <input type="hidden" name="nik" id="nik" value="<?= $rowpres->nik; ?>">
                        ...
                        <td class="text-center">
                            <?php if ($row->kegiatan == 1) {
                                foreach ($pres as $rowpres) :
                                    if ($rowpres->nik == $row->nik) { ?>
                                        <form action="<?= base_url('admin/presensi/reject/') ?>" method="POST">
                                            <input type="hidden" name="id_presensi" id="id_presensi" value="<?= $rowpres->id_presensi; ?>">
                                            <input type="hidden" name="nik" id="nik" value="<?= $rowpres->nik; ?>">
                        ...
                        <!-- <td class="text-center">
                            <?= anchor('admin/presensi/edit/' . $row->id_presensi, '<div class="btn btn-primary"><i class="fas fa-tools"></i> Edit</div>'); ?>
                        </td>
                        <td class="text-center">
                            <a onclick="return confirm('Yakin ingin menghapus data ini?')" href="<?= base_url('admin/presensi/delete/' . $row->id_presensi) ?>" class="btn btn-danger"><i class="fa fa-trash"></i> Hapus</a>
                        </td> -->
                    </tr>
                <?php endforeach; ?>
            </tbody>
